...
import { QuickShareCard } from "@/components/feed/QuickShareCard";
...
function IndexContent() {
  const { user } = useAuth();
  const { openCreate, openAuth } = useAppLayout();
    ...
    <>
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Topluluğundan en son paylaşımlar
        </p>
      </div>

      <QuickShareCard onCreate={openCreate} onAuthOpen={openAuth} />
          ...
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition ${
              filter === t.v
                ...
                : "bg-muted/60 hover:bg-accent text-foreground/80"
            }`}
          >
            {t.label}
          </button>
      ...
      {user && (
        <div className="mt-3 inline-flex rounded-full bg-muted/60 p-1">
      ...
      <div className="mt-6">
        <FeedList filter={filter} followingOnly={view === "following"} />
      </div>
    </>
  );
}
